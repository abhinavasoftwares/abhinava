...
/*
 * Each navigation item declares the module required to see it.
 *
 * Dashboard and Settings intentionally have no module requirement.
 */
const navigation = [
  {
    label: "Dashboard",
...
const visibleNavigation = useMemo(() => {
  return navigation
    .map((item) => {
      ...
      if (!hasModuleAccess(item.module, "read")) {
        return null;
      }
  ...
  /*
   * Automatically open a parent menu when one of its children
   * becomes active.
   */
  useEffect(() => {
    const nextState = {};
  ...
  const businessName =
    tenant?.business_name || "Your Boutique";
  ...
  /*
   * ------------------------------------------------------------
   * TOP BAR ENTITLEMENTS
   * ------------------------------------------------------------
   *
   * These shortcuts must follow the exact same module rules
   * as the sidebar.
   * ------------------------------------------------------------
   */

 const showEstimations =
  hasModule(CRM_MODULES.ESTIMATIONS) &&
  hasModuleAccess(CRM_MODULES.ESTIMATIONS, "read");

const showInvestments =
  hasModule(CRM_MODULES.INVESTMENTS) &&
  hasModuleAccess(CRM_MODULES.INVESTMENTS, "read");

const showLedger =
  hasModule(CRM_MODULES.LEDGER) &&
  hasModuleAccess(CRM_MODULES.LEDGER, "read");
          ...
          <div className="flex items-center gap-2.5 w-full min-w-0">
            ...
            {logoUrl ? (
              <img
                src={logoUrl}
                alt={businessName}
                className="h-8 w-8 shrink-0 rounded-lg border border-slate-200 object-contain p-0.5"
              />
            ) : (
